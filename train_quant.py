...
from model import get_bd_model, CustomReScaling
...
def main(opt):
    ...
    if opt.training == True:
        ...
        # 'Rescailing' layer check
        quantize_annotate_layer = tfmot.quantization.keras.quantize_annotate_layer
        rescaling_layer = quantize_annotate_layer(base_model.get_layer('rescaling'))
        
        # 나머지 레이어에 양자화 구성을 추가
        quantize_annotate_layer = tfmot.quantization.keras.quantize_annotate_layer
        quantized_model = quantize_annotate_layer(base_model)  # 모든 레이어에 양자화 구성 추가
